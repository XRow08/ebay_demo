  ...
  socket.emit("join", purchaseId, userId, ({ error, msgs }) => {
    ...
    msgs.forEach((e) => {
      if (e.startsWith("/uploads/")) {
        ...
        if (!e.includes("| "+sellerName+":") && !e.includes(buyerName+":")) {
...
function enviar_menssagem_resolved(msgResolve) {
  ...
  if (msgResolve.length > 0) {
    console.log(msgResolve);
    socket.emit(
      "chat message",
      purchaseId,
      pegarDataAtual() + " | " + nome_usuario + ": " + msgResolve
    );
...
socket.on("chat message", function (msg, id) {
  ...
  if (!msg.includes("| "+sellerName+":") && !msg.includes(buyerName+":")) {
    li.classList.add("admin");
  };
